...
export default function CalculationQuestion({ question, onAnswer, showResult = false, selectedAnswer }) {
  ...
  return (
    <div>
      ...
      {question.hint && !submitted && (
        <div className="mb-3">
          {showHint ? (
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800 animate-fade-in">
              💡 {question.hint}
            </div>
          ) : (
            <button
              onClick={() => setShowHint(true)}
              className="text-xs text-yellow-600 hover:text-yellow-700 bg-transparent border-none cursor-pointer"
            >
              💡 Show hint
            </button>
          )}
      ...
      <div className="flex gap-2">
        <input
          ...
          className={`flex-1 px-4 py-2.5 border-2 rounded-lg text-sm focus:outline-none font-mono ${
            submitted
        ...
        <div className={`mt-3 p-3 rounded-lg text-sm animate-fade-in ${isCorrect ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
          {isCorrect ? (
            ...
            <p className="text-red-700">
              <span className="font-medium">✕ Incorrect.</span> Answer: {accepted[0]}
            </p>
          )}
        </div>
      )}
